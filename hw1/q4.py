...
def add_irr_feature(xTrain, xTest):
    ...
    noisy_xTrain1 = np.random.normal(0, 1, size=(xTrain.shape[0], 2))
    xTrain = pd.concat([xTrain, pd.DataFrame(noisy_xTrain1, columns=["noisy1", "noisy2"])], axis=1)

    noisy_xTest1 = np.random.normal(0, 1, size=(xTest.shape[0], 2))
    xTest = pd.concat([xTest, pd.DataFrame(noisy_xTest1, columns=["noisy1", "noisy2"])], axis=1)
